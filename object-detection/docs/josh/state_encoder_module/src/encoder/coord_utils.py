...
def pixel_to_cell_float(
    ...
    """Convert pixel bbox center to continuous grid coordinates.

    Same math as pixel_to_cell but returns raw floats without int()
    truncation or clamping. Used by PositionFinder which handles
    clamping and collision resolution itself.
    ...
    Returns:
        (col_f, row_f) as continuous floats (may be outside grid bounds).
    """
    x_norm = cx / frame_w
    y_norm = cy / frame_h
