...
# Dataset path: configurable via env var, fallback to known local path
_dataset_env = os.environ.get("CR_DATASET_PATH", "")
if _dataset_env:
  path_dataset = Path(_dataset_env)
else:
  path_dataset = Path(__file__).resolve().parents[4] / "Froked-KataCR-Clash-Royale-Detection-Dataset"

if not path_dataset.exists():
  print(f"WARNING: Dataset path does not exist: {path_dataset}")

# Logs directory relative to project root (parents[2] = cr-object-detection)
path_logs = Path(__file__).resolve().parents[2] / "logs"
