...
from src.generation.plot_utils import plot_cells_PIL, plot_box_PIL, build_label2colors
from src.generation.datapath_manager import PathManager
from src.generation.constant import path_logs
from src.generation.label_list import unit2idx, idx2unit
from src.generation.state_list import state2idx, idx2state
from src.generation.generation_config import (
  ...
  color2alpha, color2bright, color2RGB, aug2prob, aug2unit, alpha_transparency, background_augment,
  component_prob, component2unit, component_cfg, important_components, option_components, bar_xy_range,
  item_cfg, drop_box, background_item_list,
  unit_scale, unit_stretch,
...
class Generator:
  ...
  def __init__(
      ...
      ally_classes: Sequence[str] = None,
    ):
    """
    Args:
      ...
      ally_classes: If set, restrict bel=0 (ally) sprites to only these class names.
        Enemy sprites (bel=1) are unrestricted. Used for fixed-deck training.
    Variables:
      map_cfg (dict):
    ...
    self.ally_classes = set(ally_classes) if ally_classes is not None else None
    for p in sorted(self.path_segment.glob('*')):
      ...
      for bel in range(2):
        # Fixed deck restriction: skip non-deck classes for ally side
        if bel == 0 and self.ally_classes is not None and p.name not in self.ally_classes:
          continue
  ...
  def build_background(self):
    ...
    if background_index is None:
      n = len(self.path_manager.search(subset='images', part='segment', name='backgrounds', regex=r"background\d+.jpg"))
    ...
    if king:
      for i in range(2):  # is enermy?
        if random.random() < king_tower_generation_ratio:
          ...
          path = self._sample_elem(self.path_manager.search(subset='images', part='segment', name='background-items', regex=r'^king-tower-ruin_\d+.png'))
      ...
      for i in range(2):  # is enermy?
        for j in range(2):  # left or right
          ...
          if p > sum(list(tower_generation_ratio.values())):  # generating tower ruin in background_itmes/ruin{i}.png
            path = self._sample_elem(self.path_manager.search(subset='images', part='segment', name='background-items', regex=r'^ruin_\d+.png'))
